...
def balanced(m):
    ...
    return balanced(l) and balanced(r) and total_weight(l) * length(left(m)) \
        == total_weight(r) * length(right(m))
...
def preorder(t):
    ...
    helper(t)
    return result
    # or return [label(t)] + sum([preorder(b) for b in branches(t)], [])
...
def quadratic(x, a, b, c):
    ...
    if x1 < s < x2:
        if a > 0:
            return interval(point, max(f1, f2))
        else:
            return interval(min(f1, f2), point)
    else:
